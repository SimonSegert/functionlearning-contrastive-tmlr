...
#from evaluations import ClassificationPerformanceExperiment, WFClassificationExperiment
...
def epoch_run(data, ds_estimator, auto_regressor, encoder, device, window_size, n_size=5, optimizer=None, train=True):
    ...
    acc = 0
    for sample in data:
        #note that original was 5*(...), here changed to 2*(...) to be consistent with tnc impl
        rnd_t = np.random.randint(2*window_size,sample.shape[-1]-2*window_size)
...
def learn_encoder(x, window_size, encoder,ds_estimator,auto_regressor,optimizer, n_size=5, n_epochs=50, data='simulation', device='cpu', n_cross_val=1):
    accuracies = []
    assert n_cross_val==1
